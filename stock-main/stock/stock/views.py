...
def categories_list(request):
    ...
    # Handling POST request for inline category update
    if request.method == 'POST' and 'edit_category' in request.POST:
        category_id = request.POST.get('category_id')
        new_name = request.POST.get('new_name')
        category = Categorie.objects.get(id=category_id)
        category.nom = new_name
        category.save()
        return redirect('categories_page')  # Reload page after saving changes
    ...
    return render(request, 'stock/liste-categories.html', context)
...
def liste_categories(request):
    ...
    # Handling inline editing (POST request with category ID and new name)
    if request.method == 'POST' and 'edit_category' in request.POST:
        category_id = request.POST.get('category_id')
        new_name = request.POST.get('new_name')
        ...
        category.nom = new_name
        category.save()
        return redirect('liste_categories')  # Reload page to show the updated list

    return render(request, 'stock/liste_categories.html', {
        'categories': categories,
        'query': query
    })
...
def send_email_view(request):
    ...
    # Check if the selected_type was passed in the GET or POST request
    if request.method == 'POST':
        selected_type = request.POST.get('recipients_choice', 'client')

    # Update recipients based on selected type
    ...
    # Handle email sending logic when POST is submitted
    if request.method == 'POST':
        ...
        # Get subject and message directly from the POST request
    ...
    # Return context with necessary data
    return render(request, 'stock/email.html', {
        'msg': msg,
        'recipients_data': recipients_data,
        'selected_type': selected_type,  # Ensure selected type is sent to the template
    })
